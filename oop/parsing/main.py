...
class BaseParser:
    ...
    def format_title(title: str) ->str:
        return " ".join(title.replace('\r', '').replace('\t', '').replace('\n', ' ').split())
    
    @staticmethod
    def format_price(price:str) ->int:
        # return " ".join(price.replace('\r', '').replace('\t', '').replace('\n', ' ').split())
        return int(re.search('\d+', price).group(0))
        
    def search(self, query:str) -> List[dict]:
        "принимает поисковое слово и возвращает список и возвращает со всеми результатами"
    ...
    def _get_product_info(self, soup:BS) -> dict:
        "метод в котором ищутся title and price продукта"
        raise NotImplemented
